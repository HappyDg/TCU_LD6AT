...
/********************  variables normal uint8 buffer section ****************/

#define NVM_START_SEC_VAR_NO_INIT_8
#include <MemMap.h>

VAR( uint8, NVM_VAR_NOINIT ) NvM_Queue_IndexStandard  = NVM_QUEUE_INDEX_RESET;

#if (NVM_API_CONFIG_CLASS != NVM_API_CONFIG_CLASS_1)

#if ( ( NVM_JOB_PRIORITIZATION == STD_ON ) && ( NVM_NUMBER_OF_IMMEDIATE_PRIORITY_BLOCKS > 0U ) )
VAR( uint8, NVM_VAR_NOINIT ) NvM_Queue_IndexImmediate;
VAR( uint8, NVM_VAR_NOINIT ) NvM_Queue_CountImmediate;
#endif
...
/******************  end of variables normal uint8 buffer section ***********/
...
FUNC(uint8, NVM_CODE) Nvm_Queue_CheckEmpty(void)
{
  ...
  DBG_NVM_QUEUE_CHECKEMPTY_ENTRY();
  ...
  DBG_NVM_QUEUE_CHECKEMPTY_EXIT(Result);
  return Result;
...
)
{
  ...
  DBG_NVM_QUEUE_CHECKFULLSTANDARD_ENTRY(BlockId);
    ...
    if ( NVM_STD_QUEUE_NOT_FULL_MULTI() )
    {
      ...
      else
      {
        ...
        if ( NvM_AdminBlockTable[NVM_BLOCK_MULTI].NvMResult == NVM_REQ_PENDING )
        {
           ...
           */
          Result = E_OK;
        } /* if ( NvM_Queue_Standard[0U].NvMBlockIdentifier == NVM_BLOCK_MULTI ) */
      } /* if ( BlockId == NVM_BLOCK_MULTI ) */
    } /* if ( NVM_STD_QUEUE_NOT_FULL_MULTI() ) */
  } /* if  ( ( NVM_STD_QUEUE_EMPTY() ) || ( NVM_STD_QUEUE_NOT_FULL() ) ) */


  DBG_NVM_QUEUE_CHECKFULLSTANDARD_EXIT(Result,BlockId);
  return Result;
...
)
{
  ...
  DBG_NVM_QUEUE_INSERTIMMEDIATE_ENTRY(BlockId, DataAddress, NvMRequestAsyncPtr);

#if ( NVM_NUMBER_OF_IMMEDIATE_PRIORITY_BLOCKS > 0U )

  if ( NvM_BlockDescriptorTable[BlockId].blockJobPriority == NVM_BLOCK_IMMEDIATE_PRIORITY )
  {
    ...
    if ( ( NvM_AdminBlockTable[BlockId].NvMResult != NVM_REQ_PENDING ) &&
         ( NVM_IMMEDIATE_QUEUE_NOT_FULL() )
       )
    {
      NvM_AdminBlockTable[BlockId].NvMResult = NVM_REQ_PENDING;

      /* Calculate the position in Immediate Queue
       * where the new request should be placed
       */
      Index = NvM_Queue_IndexImmediate + NvM_Queue_CountImmediate;
      /* Wrap around */
      Index %= NVM_SIZE_IMMEDIATE_JOB_QUEUE;

      /* insert API function parameters into the Immediate queue. */
      NvM_Queue_Immediate[Index].NvMBlockIdentifier = BlockId;
...
#endif /* #if ( NVM_NUMBER_OF_IMMEDIATE_PRIORITY_BLOCKS > 0U ) */
  {
     ...
     */
    Result = NvM_Queue_InsertStandard( BlockId, DataAddress, NvMRequestAsyncPtr );
  ...
  DBG_NVM_QUEUE_INSERTIMMEDIATE_EXIT(Result, BlockId, DataAddress, NvMRequestAsyncPtr);
  return Result;
...
)
{
  ...
  DBG_NVM_QUEUE_INSERTSTANDARD_ENTRY(BlockId,DataAddress,NvMRequestAsyncPtr);

  /* Check if the queue is full. If so, the request can not be inserted. */
  Result = NvM_Queue_CheckFullStandard(BlockId);
  ...
  if (E_OK == Result)
  {
    ...
    Priority = NvM_BlockDescriptorTable[BlockId].blockJobPriority;
    ...
    while ( ( Found != TRUE ) && ( Index != 0U ) )
    {
      ...
      CurBlockId = NvM_Queue_Standard[Index - 1U].NvMBlockIdentifier;
    ...
    NvM_Queue_Standard[Index].NvMBlockIdentifier = BlockId;
  ...
  DBG_NVM_QUEUE_INSERTSTANDARD_EXIT(Result,BlockId,DataAddress,NvMRequestAsyncPtr);
  return Result;
...
FUNC(void, NVM_CODE) NvM_Queue_RemoveFirstItem(void)
{
  ...
  DBG_NVM_QUEUE_REMOVEFIRSTITEM_ENTRY();
  ...
  if ( ( NVM_IMMEDIATE_QUEUE_NOT_EMPTY() ) &&
       ( NVM_IMMEDIATE_QUEUE_BLOCKID == NvM_GlobalBlockNumber )
     )
  {
  ...
  DBG_NVM_QUEUE_REMOVEFIRSTITEM_EXIT();
}/* end of function NvM_Queue_RemoveFirstItem */

FUNC(Std_ReturnType, NVM_CODE) NvM_Queue_PullDownFirstItem(void)
{
  NvM_QueueItemType QueueItem;
  ...
  DBG_NVM_QUEUE_PULLDOWNFIRSTITEM_ENTRY();

  /* Protect concurrent access to standard and immediate queues */
  SchM_Enter_NvM_SCHM_NVM_EXCLUSIVE_AREA_0();

#if ( ( NVM_JOB_PRIORITIZATION == STD_ON ) && ( NVM_NUMBER_OF_IMMEDIATE_PRIORITY_BLOCKS > 0U ) )
  /* Check if the current service is queued in the Immediate Queue */
  if ((NVM_IMMEDIATE_QUEUE_BLOCKID == NvM_GlobalBlockNumber) &&
      (NVM_IMMEDIATE_QUEUE_MULTIPLE_JOBS())
     )
  {
    Index = (NvM_Queue_IndexImmediate + 1U) % NVM_SIZE_IMMEDIATE_JOB_QUEUE;
    QueueItem = NvM_Queue_Immediate[NvM_Queue_IndexImmediate];
    NvM_Queue_Immediate[NvM_Queue_IndexImmediate] = NvM_Queue_Immediate[Index];
    NvM_Queue_Immediate[Index] = QueueItem;
...
#endif /* #if ( ( NVM_JOB_PRIORITIZATION == STD_ON ) && ( NVM_NUMBER_OF_IMMEDIATE_PRIORITY_BLOCKS > 0U ) ) */
  {
    if (NVM_STD_QUEUE_MULTIPLE_JOBS())
    {
      QueueItem = NvM_Queue_Standard[NvM_Queue_IndexStandard];
      NvM_Queue_Standard[NvM_Queue_IndexStandard] =
        NvM_Queue_Standard[NvM_Queue_IndexStandard - 1U];
      NvM_Queue_Standard[NvM_Queue_IndexStandard - 1U] = QueueItem;
  ...
  DBG_NVM_QUEUE_PULLDOWNFIRSTITEM_EXIT(ReturnVal);
  return ReturnVal;
}
...
{
  DBG_NVM_QUEUE_UPDATEMULTIBLOCKQUEUEITEM_ENTRY();
  ...
  if (NVM_READ_ALL_API_ID == NvM_CurrentServiceId)
  {
    ...
    if (NVM_BLOCK_CONFIG_ID == NvM_GlobalBlockNumber)
    {
      ...
      NvM_Queue_Standard[NvM_Queue_IndexStandard].NvMBlockIdentifier = NVM_BLOCK_CONFIG_ID;
    }
    else
    {
      NvM_Queue_Standard[NvM_Queue_IndexStandard].NvMRequestAsyncPtr = &NvM_ReadAll_Async_State5;

      NvM_Queue_Standard[NvM_Queue_IndexStandard].NvMBlockIdentifier = NvM_GlobalBlockNumber;
    }
  }
  else
  {
  ...
  DBG_NVM_QUEUE_UPDATEMULTIBLOCKQUEUEITEM_EXIT();
}
