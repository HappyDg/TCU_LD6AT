 ...
 */
STATIC VAR(ComM_ASR40_UserHandleArrayType, COMM_VAR) ComM_ASR40_FullComRequestors[COMM_NUM_CHANNELS];
#endif
...
{
  const uint8_least NumUser = ComM_EB_GetNumUsersOfChannel(Channel);
  uint8               UserPos;
  ...
  DBG_COMM_EVALUATECHANNEL_ENTRY(Channel);
 ...
 */
  ...
  {
    const uint8 UserOfChannel = ComM_EB_GetUsersOfChannel(Channel, UserPos);
...
#endif
#endif
  DBG_COMM_EVALUATECHANNEL_EXIT(Channel);
}
...
)
{
  ...
  DBG_COMM_RTENOTIFY_ENTRY(User,CurComMMode);

  if (ComM_RteModeUser[User] != CurComMMode)
  {
    (void) ComM_RteSwitchCbk[User](newRteMode);

    ComM_RteModeUser[User] = CurComMMode;
  }
  DBG_COMM_RTENOTIFY_EXIT(User,CurComMMode);
}
...
{
  Std_ReturnType RetVal;  /* ComM_InhibitionStatusType and ComM_ASR32_InhibitionStatusType map to the
...
{
  Std_ReturnType ret;
  DBG_COMM_ASR40_GETINHIBITIONSTATUS_ENTRY(pdav0, Status);
  ret = ComM_ASR40_GetInhibitionStatus(pdav0, Status);
  DBG_COMM_ASR40_GETINHIBITIONSTATUS_EXIT(ret, pdav0, Status);
  return ret;
}
#endif
