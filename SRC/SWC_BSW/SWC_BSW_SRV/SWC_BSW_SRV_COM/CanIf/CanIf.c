 ...
 *
 *  MISRA-2) Deviated Rule 11.5 (required)
...
/* minor version check */
#if (CANIF_SW_MINOR_VERSION < 5U)
#error CANIF_SW_MINOR_VERSION wrong (< 5U)
#endif
...
/* patch version check */
#if (CANIF_SW_PATCH_VERSION < 5U)
#error CANIF_SW_PATCH_VERSION wrong (< 5U)
#endif
...
#define CANIF_CONDITION_RX_NOT_ALLOWED( ControllerId ) \
  (    ( CanIf_CanControllerMode[(ControllerId)] != CANIF_CS_STARTED ) \
    || (    ( CanIf_PduMode[(ControllerId)] != CANIF_GET_RX_ONLINE ) \
         && (    ( CanIf_PduMode[(ControllerId)] != CANIF_GET_ONLINE ) \
              && ( CanIf_PduMode[(ControllerId)] != \
 ...
 **
 ** \param[in] pduIndex Index of the PDU in the Rx L-PDU array
 **
 ** \return Upper range CAN ID or CANIF_NO_RANGE_CONFIG if no range
 **         configuration is assigned.
 */
#define CANIF_GET_UPPER_RANGE_CANID( pduIndex ) \
  (CanIf_RxPduConfigPtr[(pduIndex)].upperRangeCanId)
...
#define CANIF_SINGLE_PDU_SW_FILTERING( pduIndex, msgCanId ) \
  ( (Std_ReturnType) \
    ( (    ( (uint32)(msgCanId) == (uint32)CanIf_RxPduConfigPtr[(pduIndex)].canId ) \
        || (    ( CANIF_GET_UPPER_RANGE_CANID( (pduIndex) ) != CANIF_NO_RANGE_CONFIG ) \
             && (    ( (uint32)CanIf_RxPduConfigPtr[(pduIndex)].canId <= (uint32)(msgCanId) ) \
                  && ( CANIF_GET_UPPER_RANGE_CANID( (pduIndex) ) >= (uint32)(msgCanId) ) \
                ) \
           ) \
      ) \
      ? E_OK \
 ...
 */
STATIC FUNC( Std_ReturnType, CANIF_CODE ) CanIf_SetPduModeInternal
  (
    uint8 ControllerId,
...
/** \brief Setting of internal PDU mode when Rx Offline mode is requested
 **
 ** This function sets the PDU mode internal for Rx OFFLINE
 **
 ** Preconditions:
 ...
 **
 */
STATIC FUNC( void, CANIF_CODE ) CanIf_SetRxOffline
  (
    uint8 ControllerId,
    CanIf_PduGetModeType OldPduMode
  );

/** \brief Setting of internal PDU mode when Rx Online mode is requested
 **
 ** This function sets the PDU mode internal for Rx ONLINE
 **
 ** Preconditions:
 ** - CanIf should be initialized
 ** - The parameter ControllerId must be checked before this function is called
 ...
 **
 */
STATIC FUNC( void, CANIF_CODE ) CanIf_SetRxOnline
  (
    uint8 ControllerId,
    CanIf_PduGetModeType OldPduMode
  );

/** \brief Setting of internal PDU mode when Tx Offline mode is requested
 **
 ** This function sets the PDU mode internal for Tx OFFLINE
 **
 ** Preconditions:
 ** - CanIf should be initialized
 ** - The parameter ControllerId must be checked before this function is called
 ...
 **
 */
STATIC FUNC( void, CANIF_CODE ) CanIf_SetTxOffline
  (
    uint8 ControllerId,
    CanIf_PduGetModeType OldPduMode
  );

/** \brief Setting of internal PDU mode when Tx Online mode is requested
 **
 ** This function sets the PDU mode internal for Tx ONLINE
 **
 ** Preconditions:
 ** - CanIf should be initialized
 ** - The parameter ControllerId must be checked before this function is called
 ...
 **
 */
STATIC FUNC( void, CANIF_CODE ) CanIf_SetTxOnline
  (
    uint8 ControllerId,
    CanIf_PduGetModeType OldPduMode
  );
...
#if (CANIF_PBCFGM_SUPPORT_ENABLED == STD_ON)
    if ( E_OK != CanIf_IsValidConfig(LocalConfigPtr))
    {
...
#endif /* CANIF_PBCFGM_SUPPORT_ENABLED == STD_ON */
    {
      ...
      for( i = 0U; i < CANIF_GET_NR_OF_CAN_CTRLS(); i++ )
      {
       ...
       ( CANIF_PUBLIC_PN_SUPPORT == STD_ON ) \
        ...
        CanIf_CanControllerFlags[i] = 0U;               /* always reset controller flags */
#endif /* ( CANIF_WAKEUP_VALIDATION_API == STD_ON ) || \
            ( CANIF_PUBLIC_TXCONFIRM_POLLING_SUPPORT == STD_ON ) || \
            ( CANIF_PUBLIC_PN_SUPPORT == STD_ON ) */

        DBG_CANIF_PDUMODE_GRP(i,(CanIf_PduMode[i]),(CANIF_GET_OFFLINE));
        CanIf_PduMode[i] = CANIF_GET_OFFLINE;           /* always reset Pdu mode */
      }
    }
  ...
  )
{
  ...
  else if( PduInfoPtr->SduLength > CANIF_MAXIMUM_CAN_DLC )
  {
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    VAR( boolean, AUTOMATIC ) TxIdValid = TRUE;   /* CanId valid flag */
#endif
    ...
    if( CanIf_CanControllerMode[ControllerId] == CANIF_CS_STARTED )
    {
      ...
      if(    ( CanIf_PduMode[ControllerId] == CANIF_GET_TX_ONLINE )
          || ( CanIf_PduMode[ControllerId] == CANIF_GET_ONLINE )
        )
      {
#if( CANIF_PUBLIC_PN_SUPPORT == STD_ON )
      ...
      else
      {
        ...
        if( ( CanIf_PduMode[ControllerId] == CANIF_GET_OFFLINE_ACTIVE ) ||
            ( CanIf_PduMode[ControllerId] == CANIF_GET_OFFLINE_ACTIVE_RX_ONLINE )
          )
        {
#if( CANIF_TX_CONFIRMATION_SUPPORT == STD_ON )
          ...
          /* CANIF740, CANIF734, CANIF072: Store Tx confirmation flag for the
  ...
  )
{
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    if( BufferIdx == CANIF_INVALID_RX_BUFFER_INDEX )
    {
      /* report DET error */
      CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                              CANIF_INSTANCE_ID,
  ...
  )
{
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    RetVal =  CanIf_SetPduModeInternal( ControllerId, PduModeRequest );
  }
    ...
    for( i = 0U; i < CANIF_GET_NR_OF_CAN_CTRLS(); i++ )
    {
      ...
      if( (CtrlConfigPtr->wakeupSourceOut & WakeupSource) != 0U )
      {
        ...
        if( (CanIf_CanControllerFlags[i] & CANIF_WAKEUP_VALIDATION_MASK) != 0U )
        {
          ...
          /* CANIF459: Clear wakeup and validation flag */
  ...
  )
{
  ...
  {
#if (CANIF_DEV_ERROR_DETECT == STD_ON)
    if(CanIf_RxIndication_DetChecks(Hrh,CanId,CanDlc,CanSduPtr) == E_OK)
#endif /* (CANIF_DEV_ERROR_DETECT == STD_ON) */
    {
      const uint16 CanIfHrhIdx = CANIF_GET_HRH_INDEX( Hrh );             /* internal HRH index */

      /* --- CanId translation ------------------------------------- */
      const uint8 ControllerId = CANIF_GET_HOH_INT_CTRL_IDX(CANIF_CFG_GET_HRH_CFG(CanIfHrhIdx));
      const Can_IdType ConfCanId = CANIF_TRANSLATE_RX_CAN_ID(CanId);
  ...
  else
  {
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
    {
...
#endif /* CANIF_CAN_REV2_COMPATIBILITY == STD_ON */
      /* check DLC */
      else if( SduLength > CANIF_MAXIMUM_CAN_DLC )
      {
        /* report DET error */
        CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                                CANIF_INSTANCE_ID,
...
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
      {
        ...
        if( ( CanIf_CanControllerMode[ControllerId] == CANIF_CS_STARTED ) &&
            ( (CanIf_PduMode[ControllerId] == CANIF_GET_TX_ONLINE) ||
              (CanIf_PduMode[ControllerId] == CANIF_GET_ONLINE) ) )
        {
          ...
          if( BufIdx != CANIF_INVALID_TX_BUFFER_INDEX )
          {
            ...
            if( CANIF_TX_BUFFER_IS_EMPTY( BufIdx ) == TRUE )
            {
              ...
              for( i = 0U; i < SduLength; i++ )
              {
                (CanIf_TxBuffer[BufIdx].data)[i] = SduDataPtr[i];
              }
  ...
  )
{
  ...
  DBG_CANIF_GETCANIFIDFROMCTRLIDX_ENTRY(CtrlId);
  ...
  DBG_CANIF_GETCANIFIDFROMCTRLIDX_EXIT((i),CtrlId);
  return (i);
}
#endif /* CANIF_SINGLE_CAN_CTRL_OPT */
  ...
  )
{
  ...
  DBG_CANIF_SETCONTROLLERMODEINTERNAL_ENTRY(ControllerId,ControllerMode);
  ...
  DBG_CANIF_SETCONTROLLERMODEINTERNAL_EXIT(RetVal,ControllerId,ControllerMode);
  return RetVal;
}
  ...
  )
{
  ...
  DBG_CANIF_BUFFERMESSAGE_ENTRY(CanTxPduId,CanHthIdx,PduInfoPtr);
  ...
  if( BufIdx != CANIF_INVALID_TX_BUFFER_INDEX )
  {
    ...
    for( i = 0U; i < PduInfoPtr->length; i++ )
    {
      (CanIf_TxBuffer[BufIdx].data)[i] = (PduInfoPtr->sdu)[i];
    }
  ...
  DBG_CANIF_BUFFERMESSAGE_EXIT(RetVal,CanTxPduId,CanHthIdx,PduInfoPtr);
  return RetVal;
}
#endif /* CANIF_PUBLIC_TX_BUFFERING == STD_ON */
...
{
  VAR( Std_ReturnType, AUTOMATIC ) RetVal = E_OK;     /* return value */
  VAR( CanIf_PduGetModeType, AUTOMATIC ) OldPduMode;  /* variable to store old channel mode */

  DBG_CANIF_SETPDUMODEINTERNAL_ENTRY(ControllerId,PduModeRequest);
  ...
  /* CANIF749: Enable partial networking filter */

  if(    ( CANIF_CFG_GET_CTRL_CFG(ControllerId).pNFilterEnabled == TRUE )
      && (    ( PduModeRequest == CANIF_SET_ONLINE )
           || ( PduModeRequest == CANIF_SET_TX_ONLINE )
         )
    )
  {
    SchM_Enter_CanIf_SCHM_CANIF_EXCLUSIVE_AREA_0();
  ...
  switch( PduModeRequest )
  {
    ...
    case CANIF_SET_RX_OFFLINE:
      /* disable Rx path */
      CanIf_SetRxOffline( ControllerId, OldPduMode );
      break;

    case CANIF_SET_RX_ONLINE:
      /* enable Rx path */
      CanIf_SetRxOnline( ControllerId, OldPduMode );
      break;

    case CANIF_SET_TX_OFFLINE:
      /* disable Tx path */
      CanIf_SetTxOffline( ControllerId, OldPduMode );
      break;

    case CANIF_SET_TX_ONLINE:
      /* enable Tx path */
      CanIf_SetTxOnline( ControllerId, OldPduMode );
      break;

    case CANIF_SET_ONLINE:
      /* set ONLINE mode */

      DBG_CANIF_PDUMODE_GRP(ControllerId,(CanIf_PduMode[ControllerId]),(CANIF_GET_ONLINE));
      CanIf_PduMode[ControllerId] = CANIF_GET_ONLINE;
      break;
    ...
    default:
      /* given mode request is unknown */
#if( CANIF_DEV_ERROR_DETECT == STD_ON )
      /* report DET error */
      CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                              CANIF_INSTANCE_ID,
                              CANIF_SID_SETPDUMODE,
                              CANIF_E_PARAM_PDUMODE
                            );
#endif /* CANIF_DEV_ERROR_DETECT == STD_ON */
      RetVal = E_NOT_OK;
      break;
  ...
  DBG_CANIF_SETPDUMODEINTERNAL_EXIT(RetVal,ControllerId,PduModeRequest);
  return RetVal;
}
  ...
  )
{
  ...
  DBG_CANIF_SETRXOFFLINE_ENTRY(ControllerId,OldPduMode);
  switch( OldPduMode )
  {
    case CANIF_GET_RX_ONLINE:

      DBG_CANIF_PDUMODE_GRP(ControllerId,(CanIf_PduMode[ControllerId]),(CANIF_GET_OFFLINE));
      CanIf_PduMode[ControllerId] = CANIF_GET_OFFLINE;
      break;
    case CANIF_GET_ONLINE:

      DBG_CANIF_PDUMODE_GRP(ControllerId,(CanIf_PduMode[ControllerId]),(CANIF_GET_TX_ONLINE));
      CanIf_PduMode[ControllerId] = CANIF_GET_TX_ONLINE;
      break;
    case CANIF_GET_OFFLINE_ACTIVE_RX_ONLINE:

      DBG_CANIF_PDUMODE_GRP(ControllerId,(CanIf_PduMode[ControllerId]),(CANIF_GET_OFFLINE_ACTIVE));
      CanIf_PduMode[ControllerId] = CANIF_GET_OFFLINE_ACTIVE;
      break;
    default:
      /* Rx path already disabled - nothing to do */
      break;
  }

  DBG_CANIF_SETRXOFFLINE_EXIT(ControllerId,OldPduMode);
}
  ...
  )
{
  ...
  DBG_CANIF_SETRXONLINE_ENTRY(ControllerId,OldPduMode);
  switch( OldPduMode )
  {
    case CANIF_GET_OFFLINE:

      DBG_CANIF_PDUMODE_GRP(ControllerId,(CanIf_PduMode[ControllerId]),(CANIF_GET_RX_ONLINE));
      CanIf_PduMode[ControllerId] = CANIF_GET_RX_ONLINE;
      break;
    ...
    case CANIF_GET_OFFLINE_ACTIVE:
      ...
      DBG_CANIF_PDUMODE_GRP(ControllerId,(CanIf_PduMode[ControllerId]),(CANIF_GET_OFFLINE_ACTIVE_RX_ONLINE));
      CanIf_PduMode[ControllerId] = CANIF_GET_OFFLINE_ACTIVE_RX_ONLINE;
      break;
    default:
      /* Rx path already enabled - nothing to do */
      break;
  }

  DBG_CANIF_SETRXONLINE_EXIT(ControllerId,OldPduMode);
}
  ...
  )
{
  ...
  DBG_CANIF_SETTXOFFLINE_ENTRY(ControllerId,OldPduMode);
  switch( OldPduMode )
  {
    case CANIF_GET_TX_ONLINE:
  ...
  DBG_CANIF_SETTXOFFLINE_EXIT(ControllerId,OldPduMode);
}
...
{
  /* enable TX_ONLINE mode according to the previous mode */

  DBG_CANIF_SETTXONLINE_ENTRY(ControllerId,OldPduMode);
  switch( OldPduMode )
  {
    case CANIF_GET_OFFLINE:
  ...
  DBG_CANIF_SETTXONLINE_EXIT(ControllerId,OldPduMode);
}
  ...
  )
{
  ...
  DBG_CANIF_SETTXOFFLINEACTIVE_ENTRY(ControllerId,OldPduMode);
  switch( OldPduMode )
  {
    case CANIF_GET_OFFLINE:
  ...
  DBG_CANIF_SETTXOFFLINEACTIVE_EXIT(ControllerId,OldPduMode);
}
...
#if( CANIF_PUBLIC_TX_BUFFERING == STD_ON )
  DBG_CANIF_SETOFFLINE_ENTRY(ControllerId, OldPduMode);
#else
  DBG_CANIF_SETOFFLINE_ENTRY(ControllerId);
#endif

  CanIf_PduMode[ControllerId] = CANIF_GET_OFFLINE;
#if( CANIF_PUBLIC_TX_BUFFERING == STD_ON )
  if( ( OldPduMode == CANIF_GET_TX_ONLINE ) ||
      ( OldPduMode == CANIF_GET_ONLINE ) )
  {
...
#if( CANIF_PUBLIC_TX_BUFFERING == STD_ON )
  DBG_CANIF_SETOFFLINE_EXIT(ControllerId, OldPduMode);
#else
  DBG_CANIF_SETOFFLINE_EXIT(ControllerId);
#endif
}
  ...
  )
{
  ...
  DBG_CANIF_WRITERXBUFFER_ENTRY(RxLPduConfig,PduInfoPtr);
  ...
  DBG_CANIF_WRITERXBUFFER_EXIT(RxLPduConfig,PduInfoPtr);
}
#endif /* ( CANIF_RX_INDICATION_CB == STD_ON ) &&
          ( CANIF_READRXPDU_DATA_API == STD_ON ) */
  ...
  )
{
  ...
  DBG_CANIF_SOFTWAREFILTERING_ENTRY(CanIfHrhIdx,CanId,PduIdxPtr);
      ...
      ( LastIdx != CANIF_INVALID_LPDU_INDEX ) )
  {
    ...
    {
      if( LastIdx == CANIF_LPDU_ID_DIRECT )
      {
    ...
    else
    {
      ...
      if( LastIdx == CANIF_LPDU_ID_DIRECT )
      {
        /* first element directly names the PDU ID */
        *PduIdxPtr = FirstIdx;
      }
      else
      {
        /* take first element from PDU list */
        *PduIdxPtr = CANIF_CFG_GET_SORTEDRXLPDU_CFG(FirstIdx);
      }
    }
  }
  else
  ...
  DBG_CANIF_SOFTWAREFILTERING_EXIT(RetVal,CanIfHrhIdx,CanId,PduIdxPtr);
  return RetVal;
}
#endif /* CANIF_RX_INDICATION_CB == STD_ON */
  ...
  )
{
  ...
  DBG_CANIF_BINARYSWFILTERING_ENTRY(FirstIdx,LastIdx,CanId,PduIdxPtr);
  ...
  /* CHECK: PARSE */
  {
    do
    {
      ...
      if( (uint32)(CanIf_RxPduConfigPtr[CANIF_CFG_GET_SORTEDRXLPDU_CFG(middle)].canId) ==
          (uint32)CanId
        )
      {
        /* ID found */
        *PduIdxPtr = CANIF_CFG_GET_SORTEDRXLPDU_CFG(middle);
      ...
      {
        if( (uint32)CanId <
            (uint32)(CanIf_RxPduConfigPtr[CANIF_CFG_GET_SORTEDRXLPDU_CFG(middle)].canId)
          )
        {
    ...
    if( RetVal == E_NOT_OK )
    {
      ...
      if(    ( middle >= (sint32)FirstIdx )
          && (    ( CANIF_GET_UPPER_RANGE_CANID( CANIF_CFG_GET_SORTEDRXLPDU_CFG(middle) ) !=
                    CANIF_NO_RANGE_CONFIG
                  )
               && ( CANIF_GET_UPPER_RANGE_CANID( CANIF_CFG_GET_SORTEDRXLPDU_CFG(middle) ) >=
                    (uint32)CanId
                  )
  ...
  DBG_CANIF_BINARYSWFILTERING_EXIT(RetVal,FirstIdx,LastIdx,CanId,PduIdxPtr);
  return RetVal;
}
  ...
  )
{
  ...
  DBG_CANIF_RXINDICATION_DETCHECKS_ENTRY(Hrh,CanId,CanDlc,CanSduPtr);
  ...
  /* parameter DLC */
  else if( CanDlc > CANIF_MAXIMUM_CAN_DLC )
  {
    /* report DET error */
    CANIF_DET_REPORT_ERROR( CANIF_MODULE_ID,
                            CANIF_INSTANCE_ID,
  ...
  DBG_CANIF_RXINDICATION_DETCHECKS_EXIT(RetCode,Hrh,CanId,CanDlc,CanSduPtr);
  return RetCode;
}
  ...
  )
{
  ...
  DBG_CANIF_DLCHANDLING_ENTRY(CanDlc,RxLPduConfigPtr,PduInfoPtr);
  ...
  else
  {
    ...
    /* check if dlc is deactivated for this L-PDU */
    if( RxLPduConfigPtr->dlc == 0U )
    {
      /* DLC check deactivated for this L-PDU -> use CanDlc */
      PduInfoPtr->SduLength = CanDlc;
    }
    else
    {
      /* if DLC check is active CanDlc must be at least as high as the
         configured DLC */
      PduInfoPtr->SduLength = RxLPduConfigPtr->dlc;
    }
  ...
  DBG_CANIF_DLCHANDLING_EXIT(RetCode,CanDlc,RxLPduConfigPtr,PduInfoPtr);
  return RetCode;
}
#endif /* CANIF_DLC_CHECK_ENABLED == STD_ON */
  ...
  )
{
  ...
  DBG_CANIF_FREECONTROLLERTXBUFFERS_ENTRY(ControllerId);
  ...
  DBG_CANIF_FREECONTROLLERTXBUFFERS_EXIT(ControllerId);
}
  ...
  )
{
  ...
  DBG_CANIF_ADDPENDINGTXREQUEST_ENTRY(CanHthIdx,CanTxPduId);
  ...
  DBG_CANIF_ADDPENDINGTXREQUEST_EXIT(CanHthIdx,CanTxPduId);
}
  ...
  ( CONST( Can_HwHandleType, AUTOMATIC ) CanHth )
{
  ...
  DBG_CANIF_SENDNEXTTXREQUEST_ENTRY(CanHth);
  ...
  do
  {
    ...
    if( PendNr > 0U )
    {
      ...
      /* CHECK: PARSE */
      {
        ...
        CanPduInfo.sdu = (CanIf_TxBuffer[BufferIdx]).data;
  ...
  DBG_CANIF_SENDNEXTTXREQUEST_EXIT(CanHth);
}
  ...
  ( CONST( Can_HwHandleType, AUTOMATIC ) CanHth )
{
  ...
  DBG_CANIF_CLEARHTHTXREQUESTS_ENTRY(CanHth);
  ...
  DBG_CANIF_CLEARHTHTXREQUESTS_EXIT(CanHth);
}
...
{
  DBG_CANIF_SENDRXINDICATION_ENTRY(userType,RxLPduId,PduInfoPtr,CanId);
  ...
  DBG_CANIF_SENDRXINDICATION_EXIT(userType,RxLPduId,PduInfoPtr,CanId);
}
#endif /* (CANIF_UL_RX_INDICATION_SUPPORT == STD_ON) || (CANIF_CDD_RX_INDICATION_SUPPORT == STD_ON) */
  ...
  )
{
  ...
  DBG_CANIF_SENDTXCONFIRMATION_ENTRY(CanTxPduId);
  ...
  DBG_CANIF_SENDTXCONFIRMATION_EXIT(CanTxPduId);
}
#endif /* CANIF_TX_CONFIRMATION_SUPPORT == STD_ON */
