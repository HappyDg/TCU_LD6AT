...
#if(CANTP_JUMPTABLE_MODE != CANTP_JUMPTABLE_CLIENT)
{
...
#endif /* CANTP_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    if(CanTpDirection == CANTP_PDU_DIR_RECEIVE)
    {
      AddressingFormat = CANTP_CFG_GET_RXNSDU_CONFIG(CanTpPduId).AddressingFormat;
      RxNPduId = CANTP_CFG_GET_RXNSDU_CONFIG(CanTpPduId).NPduIdxF;
      Ret = E_OK;
    }
    else /* must be CANTP_PDU_DIR_TRANSMIT */
    {
      /* Only for physical target address type FC frames will be received and check
         incoming N_SA.
       */
      if(CANTP_CFG_GET_TXNSDU_CONFIG(CanTpPduId).Tatype == CANTP_PHYSICAL)
      {
        AddressingFormat = CANTP_CFG_GET_TXNSDU_CONFIG(CanTpPduId).AddressingFormat;
        RxNPduId = CANTP_CFG_GET_TXNSDU_CONFIG(CanTpPduId).NPduIdFc;
        Ret = E_OK;
      }
      else
      {
        /* For functional target addressing type the N_SA will not be used. Return error. */
    ...
    if(E_OK == Ret)
    {
      /* N_SA values makes only sense with CANTP_EXTENDED configuration */
      if(CANTP_EXTENDED == AddressingFormat)
      {
        TS_AtomicAssign8(*CanTpNSaPtr, CanTp_NSaValues[RxNPduId]);
      }
      else
      {
        Ret = E_NOT_OK;
