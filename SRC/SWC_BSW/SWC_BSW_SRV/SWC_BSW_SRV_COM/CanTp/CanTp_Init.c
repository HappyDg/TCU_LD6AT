...
/* check if development error detection is enabled */
  ...
  /* perform functionality */
  {
...
#if((CANTP_DYNAMIC_NSA_ENABLED == STD_ON) || (CANTP_CHANGE_PARAMETER_REQ_API == STD_ON))
    CanTp_RxNPduLookupEntryType Index; /* index for copying N_SA values */
#endif
    ...
    */
    ...
    {
      /* stop each channel */
      for(Channel = 0U; Channel < CanTp_CfgPtr->NumberOfChannels; Channel++)
      {
...
{
  DBG_CANTP_INITCHANNELPTR_ENTRY(ChannelPtr);
  ...
  DBG_CANTP_INITCHANNELPTR_EXIT(ChannelPtr);
}
