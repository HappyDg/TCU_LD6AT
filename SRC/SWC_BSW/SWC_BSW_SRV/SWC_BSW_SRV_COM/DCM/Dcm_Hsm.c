...
FUNC(void, DCM_CODE) DCM_HSMINITINST(
  ...
  const uint8                    instIdx)
{
  CONSTP2VAR(Dcm_HsmInstType, AUTOMATIC, DCM_VAR_NOINIT) inst
  ...
  const Dcm_HsmEventType         event)
{
  ...
  if (inst->evQueueFill < sc->evQueueSize)
  {
    ...
    {
      DCM_EV_QUEUE_ENTRY(sc, instIdx, (uint8)j) = DCM_EV_QUEUE_ENTRY(sc, instIdx, (uint8)(j-1));
    }
    ++inst->evQueueFill;
  ...
  const uint8                    queueEntryNo)
{
  ...
  DBG_DCM_HSMDELFROMQUEUE_ENTRY(sc,DCM_INST(instIdx),queueEntryNo);
  /* Deviation MISRA-3 */
  ...
  DBG_DCM_HSMDELFROMQUEUE_EXIT(sc,DCM_INST(instIdx),queueEntryNo);
}
  ...
  CONSTP2CONST(Dcm_HsmTranType, AUTOMATIC, DCM_CONST) tran)
{
  ...
  DBG_DCM_HSMTRAN_ENTRY(sc,DCM_INST(instIdx),tran);
  ...
  DBG_DCM_HSMTRAN_EXIT(sc,DCM_INST(instIdx),tran);
}
  ...
  const uint8                    instIdx)
{
  ...
  DBG_DCM_HSMINITSUBSTATES_ENTRY(sc,DCM_INST(instIdx));
  ...
  DBG_DCM_HSMINITSUBSTATES_EXIT(sc,DCM_INST(instIdx));
}
  ...
  CONSTP2VAR(uint8, AUTOMATIC, AUTOMATIC)            queueEntryNo)
{
    ...
    DBG_DCM_HSMFINDEVTODISPATCH_ENTRY(sc,DCM_INST(instIdx),event,queueEntryNo);
    ...
    *queueEntryNo = (uint8)entryNo;
    ...
    DBG_DCM_HSMFINDEVTODISPATCH_EXIT(foundEventToDispatch,sc,DCM_INST(instIdx),event,queueEntryNo);
