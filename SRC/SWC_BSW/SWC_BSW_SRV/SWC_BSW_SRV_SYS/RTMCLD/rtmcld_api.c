...
uint32 Temp_MaxCPULoad = 0;

 void RTMCLD_vidCpuLoadMeasure_4SR(void)
{
	uint32 loadCup_Idle, MaxLoadCup;
	uint32 LoadCup;

	loadCup_Idle = RTMCLD_u32Load;
	MaxLoadCup = RTMCLD_u32MaxLoad4SR;

	if(RTMCLD_u32Load > Temp_MaxCPULoad)
		Temp_MaxCPULoad = RTMCLD_u32Load;
	
	LoadCup = ((loadCup_Idle * 45)/1000000);

	if(LoadCup < 1000)
	{
		RTMCLD_u32Load4SR = 1000 -LoadCup;

		if( RTMCLD_u32Load4SR > RTMCLD_u32MaxLoad4SR )
		{
			RTMCLD_u32MaxLoad4SR = RTMCLD_u32Load4SR;
		}
	}
}
