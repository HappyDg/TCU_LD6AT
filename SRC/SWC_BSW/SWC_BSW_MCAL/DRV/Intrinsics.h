...
/******************************************************************************/
IFX_LOCAL_INLINE void __ldmst_c (volatile unsigned *address, unsigned mask, unsigned value)
{
    *address = (*address & ~(mask)) | (mask & value);
}

IFX_LOCAL_INLINE uint32 __ld32 (void *addr)
{
    return *(uint32 *) addr;
}

IFX_LOCAL_INLINE void __st32 (void *addr, uint32 value)
{
    *(uint32 *) addr = value;
}
