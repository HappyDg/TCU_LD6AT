...
TASK(TASK_TIME_10MS)
{
   ...
   TST_PWD_vidMainFunction();
   TST_PWM_vidMainFunction();
   TST_ADC_vidMainFunction();
...
//	Task_HCUAPP();
	...
	//cyg 2016.09.08: add for fls debug
      Fls_17_Pmu_debug();

   //Write after HCU change the global variables. The HCU task should be before this line.
   if(MAIN_u8TestVar1 != 5)
   {
      Main_vidCAN_SngSetVle();
   }
   if(MAIN_u8TestVar1 != 1 && MAIN_u8TestVar1 != 2)//Test condition for start DCDC buck or precharge in INT(ISR_GRP_0_END_OF_CONV)
   {
...
/* !Comment: DCDC END */
   ...
   if(MAIN_u8TestVar1 != 3)//Test condition for writing IO in INT(ISR_GRP_0_END_OF_CONV)
   {
...
TASK(TASK_TIME_200MS)
{
   ...
   TST_GPT_vidMainFunction();
   TST_DET_vidMainFunction();
...
TASK(TASK_TIME_1000MS)
{
   ...
   {
		//IOHAL_vidWriteUdt_PWM_DUTY_DCDC_ISETD(10000);
		MAIN_s32TestVar4 = 0;
   }	   
   //IOHAL_vidWriteUdt_PWM_ENA_DCDC_ISETD(1);
   //
